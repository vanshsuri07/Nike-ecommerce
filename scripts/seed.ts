...
import {
  ...
  products, productVariants, ProductImageInsertSchema, productImages, type TNewProductImage,
  GenderInsertSchema, ColorInsertSchema, SizeInsertSchema, BrandInsertSchema,
  CategoryInsertSchema, CollectionInsertSchema, ProductInsertSchema,
  ProductVariantInsertSchema,
  type TNewProductVariant,
  type TNewProduct,
...
async function seed() {
  try {
    ...
    const genderRows = [
      GenderInsertSchema.parse({ label: 'Men', slug: 'men' }),
      GenderInsertSchema.parse({ label: 'Women', slug: 'women' }),
      GenderInsertSchema.parse({ label: 'Unisex', slug: 'unisex' }),
    ];
    ...
    ].map((c) => ColorInsertSchema.parse(c));
    ...
    const brand = BrandInsertSchema.parse({ name: 'Nike', slug: 'nike', logoUrl: undefined });
    {
    ...
    ].map((c) => CategoryInsertSchema.parse(c));
    ...
    const collectionRows = [
      CollectionInsertSchema.parse({ name: "Summer '25", slug: 'summer-25' }),
      CollectionInsertSchema.parse({ name: 'New Arrivals', slug: 'new-arrivals' }),
    ];
    ...
    for (let i = 0; i < productNames.length; i++) {
      ...
      for (const colors of colorChoices) {
        for (const sizes of sizeChoices) {
          ...
          const sku = `NIKE-${insertedProduct.id.slice(0, 8)}-${colors.slug}-${sizes.slug}`;
          const variant = ProductVariantInsertSchema.parse({
          ...
          const retV = await db.insert(productVariants).values(variant as TNewProductVariant).returning();
      ...
      try {
        cpSync(src, dest);
        const img: TNewProductImage = ProductImageInsertSchema.parse({
          productId: insertedProduct.id,
